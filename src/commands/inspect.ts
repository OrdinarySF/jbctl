...
export async function runInspect(adapter: ToolAdapter, toolName: string, json: boolean): Promise<string> {
  if (!toolName) {
    throw new CliError("TOOL_ERROR", "Missing tool name. Usage: jbctl inspect <tool>");
  }
