...
function readConfigFile(path: string): JetBrainsConfig {
  try {
    ...
    throw new CliError("CONNECTION_ERROR", `Failed to read config file: ${path} — ${e.message}`);
  }
}
...
export function parseCliArgs(argv: string[]): CliConfig {
  ...
  if (!project && command !== "" && command !== "help") {
    throw new CliError("CONNECTION_ERROR", "Missing required parameter: --project <path>");
  }
  ...
  if (!endpoint && command !== "" && command !== "help") {
    throw new CliError("CONNECTION_ERROR", "Missing endpoint. Provide --endpoint <url> or --config <path>");
  }
