...
describe("parseCliArgs", () => {
  ...
  test("parses doctor command with required params", () => {
    const config = parse("doctor", "--project", "/tmp", "--endpoint", "http://localhost:8080/stream");
  ...
  test("parses short flags", () => {
    const config = parse("tools", "-p", "/proj", "-e", "http://localhost:8080/stream", "-v");
  ...
  test("parses transport option", () => {
    const config = parse("doctor", "-p", "/tmp", "-e", "http://x", "--transport", "sse");
    expect(config.transport).toBe("sse");
  });

  test("parses timeout option", () => {
    const config = parse("doctor", "-p", "/tmp", "-e", "http://x", "--timeout", "5000");
    expect(config.timeout).toBe(5000);
  });

  test("parses command args for inspect", () => {
    const config = parse("inspect", "get_file_problems", "-p", "/tmp", "-e", "http://x");
    expect(config.command).toBe("inspect");
    expect(config.commandArgs).toEqual(["get_file_problems"]);
  });

  test("parses command args for call", () => {
    const config = parse("call", "get_project_modules", "-p", "/tmp", "-e", "http://x");
  ...
  test("reads --config file and extracts endpoint", () => {
    ...
    require("fs").writeFileSync(
      tmpFile,
      JSON.stringify({ type: "streamable-http", url: "http://127.0.0.1:9999/stream", headers: {} }),
    );
  ...
  test("reads SSE config and sets transport to sse", () => {
    ...
    require("fs").writeFileSync(
      tmpFile,
      JSON.stringify({ type: "sse", url: "http://127.0.0.1:9999/sse", headers: {} }),
    );
    const config = parse("doctor", "-p", "/tmp", "--config", tmpFile);
  ...
  test("--endpoint takes precedence over --config", () => {
    ...
    require("fs").writeFileSync(
      tmpFile,
      JSON.stringify({ type: "sse", url: "http://from-config/sse", headers: {} }),
    );
    const config = parse("doctor", "-p", "/tmp", "-e", "http://from-flag/stream", "--config", tmpFile);
    expect(config.endpoint).toBe("http://from-flag/stream");
  });

  test("throws on invalid config file", () => {
    expect(() => parse("doctor", "-p", "/tmp", "--config", "/nonexistent")).toThrow(CliError);
  });
});
